...
@view_config(route_name='googleplus', request_method='GET', renderer='timweb:templates/googleplus.pt', permission='author')
def get_googlePlus(request):
  ...
  if not accessToken:
    return {'feature':'Google+', 'url' : request.route_url('googleplus'), 'title':'Google+ Feature'}
  else:
    request.session.flash('You have already added the Google+ feature.')
    return HTTPFound(location=request.route_path('googleplus_confirmation'))
...
def googlePlus_callback(request):
  ...
  request.session.flash('The Google+ feature has been successfully added.')
  return HTTPFound(location=request.route_path('googleplus_confirmation'))
    

@view_config(route_name='googleplus_confirmation', request_method='GET', renderer='timweb:templates/googleplus_confirmation.pt')
def googlePlus_confirmation(request):
  ...
  res = urllib2.urlopen(req)
  personJSON = json.loads(res.read())

  firstname = personJSON['name']['givenName']
  lastname = personJSON['name']['familyName']
  profileURL = personJSON['url']
  ...
  return{'message':message,'content':''.join(posts), 'title':'Google+ Feature',
         'firstname':firstname,'lastname':lastname,'profileURL':profileURL,}
