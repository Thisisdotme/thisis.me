...
class FoursquareView(object):
  ...
  @view_config(route_name='foursquare', request_method='GET', renderer='timweb:templates/foursquare.pt', permission='author')
  def get(self):
    ...
    if 'foursquare_access_token' in self.request.session:
      ...
      try:
        req = urllib2.Request('http://%s/v1/authors/%s/features' % (self.request.registry.settings['mi.api.endpoint'],authorName))
        res = urllib2.urlopen(req)
        resJSON = json.loads(res.read())
    ...
    if not accessToken:
      return {'feature':'Foursquare', 'url' : self.request.route_url('foursquare'), 'title':'Foursquare Feature'}
    else:
  
      self.request.session.flash('You have already added the Foursquare feature.')
      return HTTPFound(location=self.request.route_path('foursquare_confirmation'))
  ...
  def callback(self):
    ...
    self.request.session.flash('The Foursquare feature has been successfully added.')
    return HTTPFound(location=self.request.route_path('foursquare_confirmation'))
    

  @view_config(route_name='foursquare_confirmation', request_method='GET', renderer='timweb:templates/foursquare_confirmation.pt')
  ...
  #        posts.append('<p>%s</p>' % post['name'])          
    ...
    return{'message':message,'content':''.join(posts), 'title':'Foursquare Feature'}
