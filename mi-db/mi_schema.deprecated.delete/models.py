...
class FeatureEvent(Base):
  ...
  def __init__(self, authorFeatureMapId, eventId, createTime, url=None, caption=None, content=None, auxillaryContent=None):
  ...
  def __repr__(self):
    return "<FeatureEvent('%s,%s,%s,%s,%s,%s,%s')>" % (self.id,self.author_feature_map_id,self.create_time,self.url,self.caption,self.content,self.auxillary_content)
