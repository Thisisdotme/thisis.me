...
class AuthorPhotoController(object):
  ...
  def get_photos(self):
    ...
    if album.service_id == Service.ME_ID:
      ...
      for event, asm, author, service_name in db_session. \
                  ...
                  limit(200):
        photo = make_photo_obj(self.db_session, self.request, event, asm, author, service_name)
        if photo:
          photos.append(photo)
    else:
    ...
    return {'type': 'photo-album', 'id': album.id, 'headline': get_album_name(album), 'photos': photos}