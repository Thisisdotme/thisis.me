...
class AuthorFeatureQuery(object):
  ...
  def getEvents(self):
    ...
    events = []  
    for fe,featureName in self.dbSession.query(FeatureEvent,Feature.feature_name).join(AuthorFeatureMap,AuthorFeatureMap.id==FeatureEvent.author_feature_map_id).join(Feature,AuthorFeatureMap.feature_id==Feature.id).filter(and_(AuthorFeatureMap.feature_id==featureId,AuthorFeatureMap.author_id==authorId)).order_by(FeatureEvent.create_time.desc()).all():
